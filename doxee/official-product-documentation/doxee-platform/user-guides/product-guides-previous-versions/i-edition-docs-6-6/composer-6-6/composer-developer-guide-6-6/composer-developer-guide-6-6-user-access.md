---
id: "5185f2bf-fddb-4c6d-9611-c6530a748913"
title: "User access"
slug: "composer-developer-guide-6-6-user-access"
...
version: 2
public_version: 2
created_at: "2025-08-20T12:24:28.034Z"
modified_at: "2025-09-01T13:00:47.096Z"
authors:
  - name: "Dominika Kassovicova"
    email: "dkassovicova@doxee.com"
source_url: "https://docs.doxee.com/docs/en/composer-developer-guide-6-6-user-access"
synced_at: "2026-01-28T21:00:48.233Z"
checksum: "564e1ce5ceed2476"
---
...
`&lt;ldap&gt;
  ...
  &lt;authenticationTypes&gt;
    ...
  &lt;/authenticationTypes&gt;
&lt;/ldap&gt;`The server element [1] defines the URL of the LDAP server. The port does not have to be specified if the standard LDAP port is used (389 for regular LDAP). If SSL should be used, the URL must begin with the `ldaps:` scheme and an SSL configuration must be included (see [SSL](/doxee-platform/docs/user-access-and-security-configuration#ssl) below).

The base DN [2] specifies the root of the LDAP directory tree that is used by Doxee. Only users and groups defined within this sub tree will be known to Doxee applications. Note that more specific DNs can be defined for users and groups separately (see [Users and Groups](/doxee-platform/docs/user-access-and-security-configuration#users-and-groups) below).
...
Individual attribute mappings may be defined for users and groups [4]. These are similar to the global attribute mappings (see [Generic user store configuration](/doxee-platform/docs/user-access-and-security-configuration#generic-user-store-configuration)) but only apply to users or groups, respectively.
...
### OpenID User Store
The OpenID user store gets user data from an OpenID Connect (OIDC) server and tokens provided by the accessing users. It can only be used correctly in conjunction with the OpenID authentication module (see [OpenID authentication (openid)](/doxee-platform/docs/security-configuration#openid-authentication-openid)).
...
As this chapter only describes the Salesforce-specific parts of the configuration, please also refer to [OpenID User Store](/doxee-platform/docs/user-access-and-security-configuration#openid-user-store) for more information.
